...
from pydantic_ai.messages import ModelResponse, ToolCallPart, ToolReturnPart, TextPart, ModelRequest
from src.agents.legitimacy_guard import legitimacy_guard, LegitimacyDependencies
...
async def test_rarity_classification():
    ...
    def mock_rarity_logic(messages, info):
        ...
        return ModelResponse(parts=[
            ToolCallPart(
                tool_name="verify_provenance",
                args={"pokemon": "celebi", "is_shiny": True}
            )
        ])

    with legitimacy_guard.override(model=FunctionModel(mock_rarity_logic)):
        result = await legitimacy_guard.run(
            "What is the rarity of a Shiny Celebi?", 
            deps=deps
        )
