...
class PIIPatterns:
    ...
    NAME_PREFIXES: ClassVar[str] = (
        r"(?:my friend|my buddy|user|trader|person named|someone called)"
    )
...
POKEMON_NAMES: frozenset[str] = frozenset({
    ...
    "alakazam", "machamp", "gengar", "gyarados", "mewtwo", "mew",
    "eevee", "snorlax", "lapras", "articuno", "zapdos", "moltres",
...
class PIIFilter:
    ...
    def _combined_pattern(self) -> re.Pattern[str]:
        ...
        named_groups = "|".join(
            f"(?P<{name}>{pattern})"
            for name, pattern in PIIPatterns.PATTERNS.items()
        )
    ...
    def filter_pii(self, text: str) -> str:
        ...
        for match in matches:
            placeholder = PIIPatterns.PLACEHOLDERS.get(
